...
def split_data(M_obs, mask, validation_fraction=0.2, test_fraction=0.1):
    ...
    Parameters:
    - M_obs: observed matrix (with missing values as placeholders)
    - mask: boolean mask of known (observed) entries (True = known)
    - validation_fraction: fraction of known entries to use as validation (from training set)
    - test_fraction: fraction of known entries to use as test (from total known)

    Returns:
    - train_mask: boolean mask for training entries
    - validation_mask: boolean mask for validation entries
...
def compute_gradient(X, M_obs, mask):
    """Gradient of squared error loss wrt X"""
    return 2 * masking(X - M_obs, mask)

def linear_minimization_oracle(grad, delta):
    """Solves min_{Y: ||Y||_* ≤ delta} <Y, grad>"""
    u, s, vt = svds(-grad, k=1, solver="arpack") # compute the SVD with ARPACK
    rank_one = np.outer(u[:, 0], vt[0, :]) # get the rank-one matrix
...
def optimal_alpha_line_search(grad, d_k, alpha_max):
    """
    Compute the optimal step size alpha for a given direction d_k.
    take the derivative of the loss function by alpha
    """
    ...
    # Clamp alpha_star to [0, alpha_bar]
...
def alpha_lipschitz(grad, d_k, L, alpha_max):
    # do the quadratic approximation via Lipschitz and take the derivative by alpha
...
def in_set(X, S):
    """ Accessory function to check if an atom is in the atom set """
